...
//
// THE CONNECTION-WRITE CENSUS (spec 4.3, launch gate 4). There are EXACTLY TWO
// sites in this client + its forked connection that write bytes to the socket:
...
// CI asserts both the COUNT and the callers of these two sites. You cannot add a
// Telegram request, a plaintext handshake message, or another socket writer
// without breaking the build.
//
// LIVE-CONNECT NOTE: the actual socket bring-up, the auth-key handshake, and the
// DC-migration dance run only inside the enclave against real Telegram DCs, and
// are validated by the fake-DC captured-wire suite (launch gate 4 runtime
// portion) during the batched Nitro rebuild. This module defines the published
// structure, the chokepoint binding, the keepalive/reconnect policy, and the
// update filter; those pure parts are unit-tested here and in the conformance
// suite. The GramJS connection primitives it builds on are pinned by lockfile.
...
export async function makeArmedTransport({ conn, onNewAuth }) {
  ...
  return {
    sender: conn.sender,
    connect: async () => { await conn.connect(); startKeepalive(); },
